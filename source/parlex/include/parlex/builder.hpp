...
struct node {
	...
	using children_t = std::vector<val<node>>;
	std::string tag;
	children_t children;
	node * parent = nullptr;
	...
	void add_child(const val<node>& child);
...
protected:
	node() = default;
	node(std::initializer_list<val<node>> const & children);
	node(std::string const & tag, std::initializer_list<val<node>> const & children);
...
struct sequence final : detail::node {
	sequence() = default;
	sequence(std::initializer_list<val<node>>&& children) : node(children) {}
	sequence(std::string const & tag, std::initializer_list<val<node>>&& children) : node(tag, children) {}
...
struct choice final : detail::node {
	choice() = default;
	choice(std::initializer_list<val<node>>&& children) : node(children) {}
	choice(std::string const & tag, std::initializer_list<val<node>>&& children) : node(tag, children) {}
...
struct optional final : detail::node {
	optional() = default;
	explicit optional(val<node> const & child) : node({ child }) {}
	optional(std::string const & tag, val<node> const & child) : node(tag, { child }) {}
...
struct repetition final : detail::node {
	repetition() = default;
	explicit repetition(val<node> const & child) : node({child}) {}
	repetition(std::string const & tag, val<node> const & child) : node(tag, {child}) {}
...
struct production {
	production(
		...
		val<detail::node> const & behavior,
