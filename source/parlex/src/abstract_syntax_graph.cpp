...
std::string parlex::abstract_syntax_graph::to_dot() const {
	...
	while (pending.size()) {
		...
		pending.pop();
		completed.insert(i);
		std::string from_name = i.r.get_id() + ":" + std::to_string(i.documentPosition) + ":" + std::to_string(i.consumed_character_count);
		auto l = table.find(i);
		if (l == table.end()) {
		...
		for (permutation const & j : l->second) {
			for (match const & k : j) {
				std::string to_name = k.r.get_id() + ":" + std::to_string(k.documentPosition) + ":" + std::to_string(k.consumed_character_count);
