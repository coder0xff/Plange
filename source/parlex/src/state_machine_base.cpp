...
void state_machine_base::on(context_ref const & c, recognizer const & r, int nextDfaState, behavior::leaf const * leaf) {
	c.owner().on(c, r, nextDfaState, leaf);
}

void state_machine_base::accept(context_ref const & c) {
	c.owner().accept(c);
}
