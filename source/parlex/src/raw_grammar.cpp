...
raw_grammar::raw_grammar(std::string const & nameOfMain) : main_production_name(nameOfMain) {
}

raw_grammar::raw_grammar(raw_grammar const & other) : grammar_base(other), main_production_name(other.main_production_name), literals(other.literals) {
	...
	for (auto const & nameAndStateMachine : other.productions) {
		...
		raw_state_machine const & sm = nameAndStateMachine.second;
		std::pair<std::map<std::string, raw_state_machine>::iterator, bool> emplaceResult = productions.emplace(
	...
	for (auto const & nameAndStateMachine : other.productions) {
		raw_state_machine const & sm = nameAndStateMachine.second;
		raw_state_machine & outProduction = productions.find(nameAndStateMachine.first)->second;
		raw_state_machine::states_t const & othersStates = sm.get_states();
...
state_machine_base const& raw_grammar::get_main_state_machine() const {
	auto i = productions.find(main_production_name);
	throw_assert(i != productions.end());
	return i->second;
}

std::map<std::string, state_machine_base const *> raw_grammar::get_state_machines() const {
...
details::string_terminal& raw_grammar::get_or_add_literal(std::u32string const & contents) {
	auto result = literals.emplace(std::piecewise_construct, forward_as_tuple(contents), forward_as_tuple(contents));
	return result.first->second;
}


state_machine_base const& raw_grammar::get_state_machine(std::string const & id) const {
...
details::string_terminal const& raw_grammar::get_literal(std::string const & id) const {
	auto i = literals.find(to_utf32(id));
	throw_assert(i != literals.end());
	return i->second;
}

raw_state_machine& raw_grammar::add_production(std::string id, size_t startState, size_t acceptStateCount, associativity assoc) {
...
raw_state_machine& raw_grammar::add_production(std::string id, size_t startState, size_t acceptStateCount, filter_function filter, associativity assoc) {
...
details::string_terminal const& raw_grammar::get_add_literal(std::u32string contents) {
...
void raw_grammar::generate_hpp(std::string grammarName, std::ostream & hpp, std::string namespace_, std::string upperCaseGrammarName) const {
...
#if 0 //to move
void raw_grammar::generate_representation(std::ostream & cpp) {
...
void raw_grammar::generate_cplusplus_code(std::string grammarName, std::string nameOfMain, std::ostream & cpp, std::ostream & hpp, std::string namespace_, std::string headerPathPrefix) const {
	...
	for (auto const & production : productions) {
		...
		raw_state_machine const & sm = production.second;
...
bool raw_grammar::test_precedence(state_machine_base const & productionA, state_machine_base const & productionB) const {
	auto i = precedences.find(static_cast<raw_state_machine const *>(&productionA));
	if (i == precedences.end()) {
		return false;
	}
	return i->second.count(static_cast<raw_state_machine const *>(&productionB)) > 0;
}

precedence_collection raw_grammar::get_precedences() const {
