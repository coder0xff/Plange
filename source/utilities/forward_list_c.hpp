...
public:
	...
	// lock free
	iterator push_front(const T & value, std::memory_order loadOrder = std::memory_order_seq_cst, std::memory_order storeOrder = std::memory_order_seq_cst) {
		auto * nodePtr = new node(value);
		return insert_node(first, nodePtr, loadOrder, storeOrder);
	}
	...
	iterator emplace_front(U&& ... params) {
		return insert_node(first, new node(std::forward<U>(params)...), std::memory_order_seq_cst, std::memory_order_seq_cst);
	}

	// lock free
	template <class... U>
	iterator emplace_front_ordered(std::memory_order loadOrder, std::memory_order storeOrder, U&& ... params) {
		return insert_node(first, new node(std::forward<U>(params)...), loadOrder, storeOrder);
	}
