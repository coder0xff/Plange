...
parlex::detail::state_machine const & plc::SUPERSET_CHAIN::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().SUPERSET_CHAIN));
	return result;
}
