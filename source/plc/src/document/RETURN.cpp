...
plc::RETURN::field_1_t_1_t plc::RETURN::field_1_t_1_t::build(parlex::detail::node const * b, parlex::detail::document::walk & w) {
...
plc::RETURN::field_2_t_1_t plc::RETURN::field_2_t_1_t::build(parlex::detail::node const * b, parlex::detail::document::walk & w) {
	auto const & children = b->children;
	auto v0 = parlex::detail::document::element<std::vector<erased<IC>>>::build(&*children[0], w);
...
plc::RETURN plc::RETURN::build(parlex::detail::ast_node const & n) {
	...
	return RETURN(std::move(v0), std::move(v1), std::move(v2));
}


parlex::detail::state_machine const & plc::RETURN::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().RETURN));
	return result;
}
