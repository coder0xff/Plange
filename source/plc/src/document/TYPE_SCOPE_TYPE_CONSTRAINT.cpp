...
plc::TYPE_SCOPE_TYPE_CONSTRAINT plc::TYPE_SCOPE_TYPE_CONSTRAINT::build(parlex::detail::ast_node const & n) {
	...
	return TYPE_SCOPE_TYPE_CONSTRAINT(std::move(v0), std::move(v1), std::move(v2));
}


parlex::detail::state_machine const & plc::TYPE_SCOPE_TYPE_CONSTRAINT::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().TYPE_SCOPE_TYPE_CONSTRAINT));
	return result;
}
