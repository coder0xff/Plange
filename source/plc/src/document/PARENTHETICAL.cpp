...
plc::PARENTHETICAL plc::PARENTHETICAL::build(parlex::detail::ast_node const & n) {
	...
	auto v_4 = parlex::detail::document::element<parlex::detail::document::text<literal_0x29_t>>::build(&*children[4], w);
	return PARENTHETICAL(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::detail::recognizer const & plc::PARENTHETICAL::recognizer() {
