...
plc::DATE_YEAR_DAY plc::DATE_YEAR_DAY::build(parlex::detail::ast_node const & n) {
	...
	return DATE_YEAR_DAY(std::move(v0), std::move(v1), std::move(v2), std::move(v3), std::move(v4), std::move(v5), std::move(v6), std::move(v7));
}


parlex::detail::state_machine const & plc::DATE_YEAR_DAY::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().DATE_YEAR_DAY));
	return result;
}
