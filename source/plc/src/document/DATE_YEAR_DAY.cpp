...
plc::DATE_YEAR_DAY plc::DATE_YEAR_DAY::build(parlex::details::ast_node const & n) {
	...
	return DATE_YEAR_DAY(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4), std::move(v_5), std::move(v_6), std::move(v_7));
}


parlex::details::recognizer const & plc::DATE_YEAR_DAY::recognizer() {
	return plange_grammar::get().DATE_YEAR_DAY.get_recognizer();
}
