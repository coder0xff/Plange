...
plc::INTERSECTION plc::INTERSECTION::build(parlex::details::ast_node const & n) {
	...
	auto v_2 = parlex::details::document::element<parlex::details::document::text<literal_0xE20x880xA9_t>>::build(&*children[2], w);
	...
	return INTERSECTION(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::details::recognizer const & plc::INTERSECTION::recognizer() {
	return plange_grammar::get().INTERSECTION.get_recognizer();
}
