...
ASM_EXPRESSION ASM_EXPRESSION::build(std::u32string const & document, parlex::details::ast_node const & n) {
	static auto const & b = plange_grammar::get().ASM_EXPRESSION.get_behavior();
	parlex::details::document::walk w{ n.children.cbegin(), n.children.cend() };
	return ASM_EXPRESSION(parlex::details::document::element<ASM_EXPRESSION_base>::build(document, b, w));
}
