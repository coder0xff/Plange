...
parlex::detail::state_machine const & plc::TYPE_SCOPE_TYPE_CONSTRAINT_CHAIN::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().TYPE_SCOPE_TYPE_CONSTRAINT_CHAIN));
	return result;
}
