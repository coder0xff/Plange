...
plc::SET_COMPREHENSION plc::SET_COMPREHENSION::build(parlex::detail::ast_node const & n) {
	...
	return SET_COMPREHENSION(n.document_position, n.consumed_character_count, std::move(v0), std::move(v1), std::move(v2), std::move(v3), std::move(v4), std::move(v5), std::move(v6), std::move(v7), std::move(v8));
}
