...
plc::VECTOR_NORM plc::VECTOR_NORM::build(std::u32string const & document, parlex::details::ast_node const & n) {
	...
	assert(w.pos != w.end); ++w.pos; //|| 
	return VECTOR_NORM(std::move(v_0), std::move(v_1), std::move(v_2));
}
