...
plc::IN plc::IN::build(parlex::details::ast_node const & n) {
	static auto const * b = &plange_grammar::get().IN.get_behavior();
	...
	return IN(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::details::recognizer const & plc::IN::recognizer() {
	return plange_grammar::get().IN.get_recognizer();
}
