...
plc::CASTS plc::CASTS::build(std::u32string const & document, parlex::details::ast_node const & n) {
	...
	assert(w.pos != w.end); ++w.pos; //casts 
	...
	return CASTS(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3));
}
