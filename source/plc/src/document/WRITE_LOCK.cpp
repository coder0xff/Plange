...
plc::WRITE_LOCK plc::WRITE_LOCK::build(std::u32string const & document, parlex::details::ast_node const & n) {
	static auto const & b = plange_grammar::get().WRITE_LOCK.get_behavior();
	parlex::details::document::walk w{ n.children.cbegin(), n.children.cend() };
	auto const & children = b.get_children();
	assert(w.pos != w.end); ++w.pos; //write_lock 
	...
	return WRITE_LOCK(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3));
}
