...
plc::WRITE_LOCK plc::WRITE_LOCK::build(parlex::detail::ast_node const & n) {
	...
	return WRITE_LOCK(std::move(v0), std::move(v1), std::move(v2), std::move(v3), std::move(v4));
}


parlex::detail::state_machine const & plc::WRITE_LOCK::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().WRITE_LOCK));
	return result;
}
