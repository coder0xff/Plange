...
plc::NOT_IN plc::NOT_IN::build(parlex::detail::ast_node const & n) {
	...
	return NOT_IN(std::move(v0), std::move(v1), std::move(v2), std::move(v3), std::move(v4));
}


parlex::detail::state_machine const & plc::NOT_IN::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().NOT_IN));
	return result;
}
