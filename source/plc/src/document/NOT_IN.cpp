...
plc::NOT_IN plc::NOT_IN::build(parlex::details::ast_node const & n) {
	static auto const * b = &plange_grammar::get().NOT_IN.get_behavior();
	...
	return NOT_IN(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::details::recognizer const & plc::NOT_IN::recognizer() {
	return plange_grammar::get().NOT_IN.get_recognizer();
}
