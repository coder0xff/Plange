...
plc::MEMBER_ACCESS plc::MEMBER_ACCESS::build(std::u32string const & document, parlex::details::ast_node const & n) {
	...
	auto v_3 = parlex::details::document::element<erased<IDENTIFIER>>::build(document, *children[4], w);
	return MEMBER_ACCESS(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3));
}
