...
parlex::detail::state_machine const & plc::GREATER_CHAIN_LOOP::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().GREATER_CHAIN_LOOP));
	return result;
}
