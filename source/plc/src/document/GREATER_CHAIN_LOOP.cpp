...
plc::GREATER_CHAIN_LOOP::field_3_t_1_t plc::GREATER_CHAIN_LOOP::field_3_t_1_t::build(parlex::details::behavior::node const * b, parlex::details::document::walk & w) {
	auto const & children = b->get_children();
	auto v_0 = parlex::details::document::element<std::vector<erased<IC>>>::build(&*children[0], w);
	auto v_1 = parlex::details::document::element<erased<GREATER_CHAIN_LOOP>>::build(&*children[1], w);
	return field_3_t_1_t(std::move(v_0), std::move(v_1));
}

plc::GREATER_CHAIN_LOOP plc::GREATER_CHAIN_LOOP::build(parlex::details::ast_node const & n) {
...
parlex::details::recognizer const & plc::GREATER_CHAIN_LOOP::recognizer() {
	return plange_grammar::get().GREATER_CHAIN_LOOP.get_recognizer();
}
