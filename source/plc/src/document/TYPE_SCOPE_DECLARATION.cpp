...
parlex::detail::state_machine const & plc::TYPE_SCOPE_DECLARATION::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().TYPE_SCOPE_DECLARATION));
	return result;
}
