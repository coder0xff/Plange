...
plc::INEQUALITY plc::INEQUALITY::build(parlex::details::ast_node const & n) {
	static auto const * b = &plange_grammar::get().INEQUALITY.get_behavior();
	...
	return INEQUALITY(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::details::recognizer const & plc::INEQUALITY::recognizer() {
	return plange_grammar::get().INEQUALITY.get_recognizer();
}
