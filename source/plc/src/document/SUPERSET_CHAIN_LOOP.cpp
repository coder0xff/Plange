...
plc::SUPERSET_CHAIN_LOOP::field_4_t_1_t plc::SUPERSET_CHAIN_LOOP::field_4_t_1_t::build(parlex::detail::node const * b, parlex::detail::document::walk & w) {
...
plc::SUPERSET_CHAIN_LOOP plc::SUPERSET_CHAIN_LOOP::build(parlex::detail::ast_node const & n) {
	...
	return SUPERSET_CHAIN_LOOP(std::move(v0), std::move(v1), std::move(v2), std::move(v3));
}


parlex::detail::state_machine const & plc::SUPERSET_CHAIN_LOOP::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().SUPERSET_CHAIN_LOOP));
	return result;
}
