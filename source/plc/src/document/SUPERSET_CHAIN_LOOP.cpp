...
plc::SUPERSET_CHAIN_LOOP::field_4_t plc::SUPERSET_CHAIN_LOOP::field_4_t::build(parlex::detail::node const * b, parlex::detail::document::walk & w) {
	auto const & children = b->children;
	auto v0 = parlex::detail::document::element<std::vector<erased<IC>>>::build(&*children[0], w);
	auto v1 = parlex::detail::document::element<erased<SUPERSET_CHAIN_LOOP>>::build(&*children[1], w);
...
plc::SUPERSET_CHAIN_LOOP plc::SUPERSET_CHAIN_LOOP::build(parlex::detail::ast_node const & n) {
	...
	return SUPERSET_CHAIN_LOOP(n.document_position, n.consumed_character_count, std::move(v0), std::move(v1), std::move(v2), std::move(v3));
}


parlex::detail::acceptor const & plc::SUPERSET_CHAIN_LOOP::acceptor() {
	static auto const & result = *static_cast<parlex::detail::acceptor const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().SUPERSET_CHAIN_LOOP));
	return result;
}
