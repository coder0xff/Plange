...
plc::EQUALITY_CHAIN plc::EQUALITY_CHAIN::build(parlex::details::ast_node const & n) {
	...
	auto v_2 = parlex::details::document::element<erased<EQUALITY_CHAIN_LOOP>>::build(&*children[2], w);
	return EQUALITY_CHAIN(std::move(v_0), std::move(v_1), std::move(v_2));
}


parlex::details::recognizer const & plc::EQUALITY_CHAIN::recognizer() {
