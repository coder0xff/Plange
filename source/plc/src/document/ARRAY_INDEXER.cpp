...
parlex::detail::state_machine const & plc::ARRAY_INDEXER::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().ARRAY_INDEXER));
	return result;
}
