...
plc::LOCK plc::LOCK::build(parlex::detail::ast_node const & n) {
	...
	auto v_0 = parlex::detail::document::element<parlex::detail::document::text<literal_lock_t>>::build(&*children[0], w);
	...
	auto v_4 = parlex::detail::document::element<erased<BLOCK>>::build(&*children[4], w);
	return LOCK(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::detail::recognizer const & plc::LOCK::recognizer() {
