...
plc::EQUALITY_CHAIN_LOOP::field_2_t_1_t plc::EQUALITY_CHAIN_LOOP::field_2_t_1_t::build(std::u32string const & document, parlex::details::behavior::node const & b, parlex::details::document::walk & w) {
	auto const & children = b.get_children();
	auto v_0 = parlex::details::document::element<std::vector<erased<IC>>>::build(document, *children[0], w);
	auto v_1 = parlex::details::document::element<erased<EQUALITY_CHAIN_LOOP>>::build(document, *children[1], w);
	return field_2_t_1_t(std::move(v_0), std::move(v_1));
}

plc::EQUALITY_CHAIN_LOOP plc::EQUALITY_CHAIN_LOOP::build(std::u32string const & document, parlex::details::ast_node const & n) {
	...
	return EQUALITY_CHAIN_LOOP(std::move(v_0), std::move(v_1), std::move(v_2));
}
