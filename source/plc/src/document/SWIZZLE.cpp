...
plc::SWIZZLE plc::SWIZZLE::build(parlex::details::ast_node const & n) {
	static auto const * b = &plange_grammar::get().SWIZZLE.get_behavior();
	...
	return SWIZZLE(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::details::recognizer const & plc::SWIZZLE::recognizer() {
	return plange_grammar::get().SWIZZLE.get_recognizer();
}
