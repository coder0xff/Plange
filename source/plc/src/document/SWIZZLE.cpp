...
plc::SWIZZLE plc::SWIZZLE::build(parlex::detail::ast_node const & n) {
	...
	return SWIZZLE(std::move(v0), std::move(v1), std::move(v2), std::move(v3), std::move(v4));
}


parlex::detail::state_machine const & plc::SWIZZLE::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().SWIZZLE));
	return result;
}
