...
plc::SUBSET_CHAIN plc::SUBSET_CHAIN::build(parlex::detail::ast_node const & n) {
	...
	auto v2 = parlex::detail::document::element<val<SUBSET_CHAIN_LOOP>>::build(&*children[2], w);
	return SUBSET_CHAIN(n.document_position, n.consumed_character_count, std::move(v0), std::move(v1), std::move(v2));
}
