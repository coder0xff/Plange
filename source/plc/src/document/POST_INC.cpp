...
plc::POST_INC plc::POST_INC::build(parlex::detail::ast_node const & n) {
	...
	return POST_INC(std::move(v0), std::move(v1), std::move(v2));
}


parlex::detail::state_machine const & plc::POST_INC::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().POST_INC));
	return result;
}
