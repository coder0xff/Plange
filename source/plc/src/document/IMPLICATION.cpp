...
plc::IMPLICATION plc::IMPLICATION::build(parlex::details::ast_node const & n) {
	static auto const * b = &plange_grammar::get().IMPLICATION.get_behavior();
	...
	auto v_2 = parlex::details::document::element<std::variant<
		parlex::details::document::text<literal_0xE20x870x92_t>,
	...
	return IMPLICATION(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::details::recognizer const & plc::IMPLICATION::recognizer() {
	return plange_grammar::get().IMPLICATION.get_recognizer();
}
