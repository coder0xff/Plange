...
plc::FUNCTION_MODIFIER_STABILITY::field_1_t_1_t plc::FUNCTION_MODIFIER_STABILITY::field_1_t_1_t::build(std::u32string const & document, parlex::details::behavior::node const & b, parlex::details::document::walk & w) {
	auto const & children = b.get_children();
	auto v_0 = parlex::details::document::element<erased<ICR>>::build(document, *children[0], w);
	auto v_1 = parlex::details::document::element<erased<FUNCTION_MODIFIER_1>>::build(document, *children[1], w);
	return field_1_t_1_t(std::move(v_0), std::move(v_1));
}

plc::FUNCTION_MODIFIER_STABILITY plc::FUNCTION_MODIFIER_STABILITY::build(std::u32string const & document, parlex::details::ast_node const & n) {
