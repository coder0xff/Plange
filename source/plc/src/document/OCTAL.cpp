...
plc::OCTAL plc::OCTAL::build(parlex::detail::ast_node const & n) {
	...
	return OCTAL(std::move(v0), std::move(v1), std::move(v2));
}


parlex::detail::state_machine const & plc::OCTAL::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().OCTAL));
	return result;
}
