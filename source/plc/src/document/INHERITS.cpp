...
plc::INHERITS plc::INHERITS::build(std::u32string const & document, parlex::details::ast_node const & n) {
	...
	assert(w.pos != w.end); ++w.pos; //inherits 
	...
	return INHERITS(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3));
}
