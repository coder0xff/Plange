...
BINARY_OP BINARY_OP::build(std::u32string const & document, parlex::details::ast_node const & n) {
	static auto const & b = plange_grammar::get().BINARY_OP.get_behavior();
	parlex::details::document::walk w{ n.children.cbegin(), n.children.cend() };
	return BINARY_OP(parlex::details::document::element<BINARY_OP_base>::build(document, b, w));
}
