...
parlex::detail::state_machine const & plc::MEMBER_OFFSET::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().MEMBER_OFFSET));
	return result;
}
