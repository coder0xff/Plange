...
plc::ADD_SUB plc::ADD_SUB::build(parlex::detail::ast_node const & n) {
	...
	return ADD_SUB(std::move(v0), std::move(v1), std::move(v2), std::move(v3), std::move(v4));
}


parlex::detail::state_machine const & plc::ADD_SUB::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().ADD_SUB));
	return result;
}
