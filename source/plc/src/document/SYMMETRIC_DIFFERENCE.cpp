...
parlex::detail::state_machine const & plc::SYMMETRIC_DIFFERENCE::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().SYMMETRIC_DIFFERENCE));
	return result;
}
