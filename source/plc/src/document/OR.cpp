...
plc::OR plc::OR::build(parlex::details::ast_node const & n) {
	static auto const * b = &plange_grammar::get().OR.get_behavior();
	...
	return OR(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::details::recognizer const & plc::OR::recognizer() {
	return plange_grammar::get().OR.get_recognizer();
}
