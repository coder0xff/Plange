...
plc::IS plc::IS::build(std::u32string const & document, parlex::details::ast_node const & n) {
	...
	return IS(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3));
}
