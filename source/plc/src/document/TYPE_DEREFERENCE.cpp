...
plc::TYPE_DEREFERENCE plc::TYPE_DEREFERENCE::build(parlex::detail::ast_node const & n) {
	...
	return TYPE_DEREFERENCE(std::move(v0), std::move(v1), std::move(v2), std::move(v3), std::move(v4));
}


parlex::detail::state_machine const & plc::TYPE_DEREFERENCE::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().TYPE_DEREFERENCE));
	return result;
}
