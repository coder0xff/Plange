...
plc::TYPE_DEREFERENCE plc::TYPE_DEREFERENCE::build(std::u32string const & document, parlex::details::ast_node const & n) {
	...
	assert(w.pos != w.end); ++w.pos; //> 
	return TYPE_DEREFERENCE(std::move(v_0), std::move(v_1), std::move(v_2));
}
