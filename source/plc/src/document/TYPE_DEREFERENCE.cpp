...
plc::TYPE_DEREFERENCE plc::TYPE_DEREFERENCE::build(parlex::details::ast_node const & n) {
	static auto const * b = &plange_grammar::get().TYPE_DEREFERENCE.get_behavior();
	...
	return TYPE_DEREFERENCE(std::move(v_0), std::move(v_1), std::move(v_2), std::move(v_3), std::move(v_4));
}


parlex::details::recognizer const & plc::TYPE_DEREFERENCE::recognizer() {
	return plange_grammar::get().TYPE_DEREFERENCE.get_recognizer();
}
