...
plc::POST_DEC plc::POST_DEC::build(parlex::detail::ast_node const & n) {
	...
	return POST_DEC(std::move(v0), std::move(v1), std::move(v2));
}


parlex::detail::state_machine const & plc::POST_DEC::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().POST_DEC));
	return result;
}
