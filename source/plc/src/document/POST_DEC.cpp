...
plc::POST_DEC plc::POST_DEC::build(parlex::details::ast_node const & n) {
	...
	auto v_2 = parlex::details::document::element<parlex::details::document::text<literal_0x2D0x2D_t>>::build(&*children[2], w);
	return POST_DEC(std::move(v_0), std::move(v_1), std::move(v_2));
}


parlex::details::recognizer const & plc::POST_DEC::recognizer() {
