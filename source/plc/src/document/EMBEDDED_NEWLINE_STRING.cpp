...
parlex::detail::state_machine const & plc::EMBEDDED_NEWLINE_STRING::state_machine() {
	static auto const & result = *static_cast<parlex::detail::state_machine const *>(&plange_grammar::get().get_recognizer(plange_grammar::get().EMBEDDED_NEWLINE_STRING));
	return result;
}
