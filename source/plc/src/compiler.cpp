...
symbol_table compute_assignment_chain_symbol_table(source_code const& source, std::vector<parlex::match> const& parts) {
	symbol_table results;
...
symbol_table compute_statement_symbol_table(source_code const& source, std::vector<parlex::match> const& parts, scope & parent, llvm::BasicBlock & unwindBlock, llvm::IRBuilder<> & builder) {
	auto p = parts[0];
	if (&p.r == &ASSIGNMENT_CHAIN) {
...
symbol_table compute_scope_symbol_table(source_code const& source, std::vector<parlex::match> const& parts, scope& parent, llvm::BasicBlock & unwindBlock, llvm::IRBuilder<> & builder) {
	symbol_table results;
	for (auto part : parts) {
		if (&part.r == &STATEMENT) {
