...
struct FUNCTION {
	...
	struct field_2_t {
		...
		explicit field_2_t
			(val<FUNCTION_MODIFIER_0> && field_1, std::vector<val<IC>> && field_2)
	...
	explicit FUNCTION
		(int32_t documentPosition, int32_t consumedCharacterCount, std::optional<field_1_t> && field_1, std::optional<field_2_t> && field_2, val<BLOCK> && block)
