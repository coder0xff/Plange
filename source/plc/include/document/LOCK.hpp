...
struct LOCK {
	...
	explicit LOCK
		(int32_t documentPosition, int32_t consumedCharacterCount, parlex::detail::document::text<literal_lock_t> && dont_care0, std::vector<val<IC>> && field_1, val<PARENTHETICAL> && field_2, std::vector<val<IC>> && field_3, val<BLOCK> && field_4)
