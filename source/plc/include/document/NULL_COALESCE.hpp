...
struct NULL_COALESCE {
	...
	explicit NULL_COALESCE
		(int32_t documentPosition, int32_t consumedCharacterCount, val<EXPRESSION> && field_1, std::vector<val<IC>> && field_2, parlex::detail::document::text<literal_0x3F0x3F_t> && dont_care2, std::vector<val<IC>> && field_3, val<EXPRESSION> && field_4)
