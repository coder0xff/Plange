...
struct ASM_FUNCTION {
	...
	explicit ASM_FUNCTION(
		std::optional<field_1_t_1_t> && field_1, std::optional<erased<FUNCTION_MODIFIER_0>> && field_2, std::vector<erased<IC>> && field_3, erased<ASM_SCOPE> && field_4) : field_1(std::move(field_1)), field_2(std::move(field_2)), field_3(std::move(field_3)), field_4(std::move(field_4)) {}

	ASM_FUNCTION(ASM_FUNCTION const & other) = default;
	ASM_FUNCTION(ASM_FUNCTION && move) = default;

	static ASM_FUNCTION build(std::u32string const & document, parlex::details::ast_node const & n);
