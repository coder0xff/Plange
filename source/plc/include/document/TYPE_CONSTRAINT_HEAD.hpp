...
struct TYPE_CONSTRAINT_HEAD {
	...
	explicit TYPE_CONSTRAINT_HEAD(
		...
		std::variant<
			...
			erased<VOLATILE_IMPLICIT_TYPE_DEREFERENCE>
		> && type_dereference
	) : field_1(std::move(field_1)), field_2(std::move(field_2)), field_3(std::move(field_3)), field_4(std::move(field_4)), field_5(std::move(field_5)), type_dereference(std::move(type_dereference)) {}

	TYPE_CONSTRAINT_HEAD(TYPE_CONSTRAINT_HEAD const & other) = default;
	TYPE_CONSTRAINT_HEAD(TYPE_CONSTRAINT_HEAD && move) = default;

	static TYPE_CONSTRAINT_HEAD build(parlex::details::behavior::node const & b, parlex::details::ast_node const & n);
