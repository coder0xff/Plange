...
struct TYPE_SCOPE_ASSIGNMENT_NODE {
	...
	struct field_2_t {
		struct field_1_t {
			...
			explicit field_1_t
				(val<MEMBER_OFFSET> && field_1, std::vector<val<IC>> && ic)
		...
		explicit field_2_t
			(val<IDENTIFIER_SPECIFICATION> && identifier_specification, std::optional<field_1_t> && field_1, std::vector<val<IC>> && field_2, val<TYPE_SCOPE_ASSIGNMENT_NODE> && next)
			: identifier_specification(std::move(identifier_specification)), field_1(std::move(field_1)), field_2(std::move(field_2)), next(std::move(next)) {}
...
> && field_1, std::vector<val<IC>> && ic, std::variant<
	...
	field_2_t
> && field_2)
		: document_position(documentPosition), consumed_character_count(consumedCharacterCount), field_1(std::move(field_1)), ic(std::move(ic)), field_2(std::move(field_2)) {}

	TYPE_SCOPE_ASSIGNMENT_NODE(TYPE_SCOPE_ASSIGNMENT_NODE const & other) = default;
	TYPE_SCOPE_ASSIGNMENT_NODE(TYPE_SCOPE_ASSIGNMENT_NODE && move) = default;
