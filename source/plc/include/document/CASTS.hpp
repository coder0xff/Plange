...
struct CASTS {
	...
	explicit CASTS
		(int32_t documentPosition, int32_t consumedCharacterCount, val<EXPRESSION> && field_1, std::vector<val<IC>> && field_2, parlex::detail::document::text<literal_casts_t> && dont_care2, std::vector<val<IC>> && field_3, val<EXPRESSION> && field_4)
