...
struct ADDITION {
	...
	explicit ADDITION
		(int32_t documentPosition, int32_t consumedCharacterCount, val<EXPRESSION> && field_1, std::vector<val<IC>> && field_2, parlex::detail::document::text<literal_0x2B_t> && dont_care2, std::vector<val<IC>> && field_3, val<EXPRESSION> && field_4)
